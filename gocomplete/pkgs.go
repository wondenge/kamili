...
func predictPackages(a kamili.Args) (prediction []string) {
...
func predictLocalAndSystem(a kamili.Args) []string {
	localDirs := kamili.PredictFilesSet(listPackages(a.Directory())).Predict(a)
	// System directories are not actual file names, for example: 'github.com/wondenge/kamili' could
	// be the argument, but the actual filename is in $GOPATH/src/github.com/wondenge/kamili'. this
	// is the reason to use the PredictSet and not the PredictDirs in this case.
	s := systemDirs(a.Last)
	sysDirs := kamili.PredictSet(s...).Predict(a)
...
func listPackages(dir string) (directories []string) {
	...
	if err != nil {
		kamili.Log("failed reading directory %s: %s", dir, err)
		return
	}
	...
	for _, p := range paths {
		...
		if err != nil {
			kamili.Log("failed importing directory %s: %s", p, err)
			continue
		}
